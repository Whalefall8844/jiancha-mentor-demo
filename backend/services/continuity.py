    ...
    *,
    ...
    with transaction() as connection:
        ...
        if from_member_id:
            ...
            if source is None:
                raise ValueError("交接发起人不属于当前项目")
        else:
            source = connection.execute(
                "SELECT * FROM project_members WHERE project_id = ? AND display_name = ? AND role = 'CRA' LIMIT 1",
                (visit["project_id"], visit["cra_name"]),
            ).fetchone()
        connection.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'cra_initiated', '', '', 'completed', ?, ?)
            """,
            (handover_id, visit["project_id"], visit_id, source["id"] if source else None, target["id"], note.strip(), actor_name, timestamp),
        )
        connection.execute(
        ...
        _audit(
            ...
            action="completed",
            actor_name=actor_name,
            detail={"from_member_id": source["id"] if source else None, "to_member_id": target["id"], "to_member_name": target["display_name"]},
        )
